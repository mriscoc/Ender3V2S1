...
#if ProUI && HAS_MESH
  void LiveMeshPoints() { ProEx.DrawMeshPoints(true, CurrentMenu->line(), HMI_value.Value); }
  void SetMeshPoints() {
...
int8_t HMI_GetInt(const int32_t lo, const int32_t hi) {
  ...
  if (encoder_diffState != ENCODER_DIFF_NO) {
    if (Apply_Encoder(encoder_diffState, HMI_value.Value)) {
      EncoderRate.enabled = false;
      DWINUI::Draw_Signed_Int(HMI_data.Text_Color, HMI_data.Background_Color, 4 , VALX, MBASE(CurrentMenu->line()) - 1, HMI_value.Value);
      checkkey = Menu;
      return 2;
    }
    LIMIT(HMI_value.Value, lo, hi);
    DWINUI::Draw_Signed_Int(HMI_data.Text_Color, HMI_data.Selected_Color, 4 , VALX, MBASE(CurrentMenu->line()) - 1, HMI_value.Value);
    return 1;
  }
  return 0;
}
...
#if HAS_BED_PROBE
// C12 Read a Z level with a probe for a given X,Y
void DWIN_C12() {
    char cmd[31] = "";
    char str_1[6], str_2[6],str_3[6] = "";
    #define fmt "X:%s, Y:%s, Z:%s"
    const float xpos = parser.seenval('X') ? parser.value_float() : 0;
    const float ypos = parser.seenval('Y') ? parser.value_float() : 0;
    float zval = NAN;
...
void DWIN_Gcode(const int16_t codenum) {
  switch(codenum) {
    ...
    #if HAS_BED_PROBE
    case  12: DWIN_C12(); break;            // Read a Z level with a probe for a given X,Y
    #endif
